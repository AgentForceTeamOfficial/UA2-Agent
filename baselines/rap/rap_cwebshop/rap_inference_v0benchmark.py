...
"""
    ...
    file_name      = f"./rap_w_history_cwebshopRunTimeEnv_v0_U{USER_NUM}_T{TASK_NUM}_{datetime.now().strftime('%Y-%m-%d-%H-%M')}.txt"
    url_log_name   = f"./rap_w_history_cwebshopRunTimeEnv_v0_U{USER_NUM}_T{TASK_NUM}_{datetime.now().strftime('%Y-%m-%d-%H-%M')}_action_url.txt"
    avg_reward   = []
    success_rate = []
    avg_time     = []
    avg_money    = []
